 ...
 */
[[nodiscard]] Errors::ChangeLine changeLine(const std::string &fileName,
  ...
  while (1) {
    ...
    line[posStart + what] == '0' ? line[posStart + what] = '1'
                                 : line[posStart + what] = '0';
    file << line + '\n';
    break;
  }
...
getPossibleHeroes(const std::string &fileName, const bool suppress) {
  std::ifstream file(fileName);
  if (!file) {
    if (!suppress)
      QMessageBox::critical(nullptr, "Cannot open file",
                            "For some reason BCR cannot open file for reading");
    return {};
  }
 ...
 */
[[nodiscard, deprecated]] QString *
d_getPossibleSkills(int numCommand, QString *fighters, const bool suppress) {
  ...
  for (int i = 0; i < 18; i++) {
    ...
    QString heroName = QString::fromStdString(
        line.substr(0, line.find(":"))); // cut line to get only hero's name
    bool found = false;
    for (int j = 0; j < 4; j++)
...
std::array<QString, 4>
getPossibleSkills(const std::string &fileName,
                  const std::array<std::string, 4> fighters,
                  const bool suppress) {
